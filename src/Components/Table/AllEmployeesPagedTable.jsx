...
const AllEmployeesTablePaged = ({ data }) => {
  ...
  const handleFilterChange = (key, value) => {
    ...
    const filtered = data.filter((entry) => {
      return tableKeys.every((tKey) => {
        //returns a boolean if any of the table elements contain the entered value
        const filterValue = newFilterValues[tKey];
        if (!filterValue) return true; // No filter applied
        return String(entry[tKey])
          .toLowerCase()
  ...
  const handleSort = (key) => {
    ...
    const newOrder =
      sorting.key === key && sorting.order === "asc" ? "desc" : "asc";
